...
export default function CalendarMain({
  ...
  setIsScheduleModalOpen, handlePrevWeek, handleNextWeek, handleToday, handleEventClick, handleEventDragStart, handleEventResize,
  ...
  // ★ 追加：週表示における「1日分の正確な幅」を計算するステート
  const [dayWidth, setDayWidth] = useState(192);

  // ★ 追加：画面やサイドバーのサイズが変わるたびに幅を再計算して7等分する
  useEffect(() => {
    if (viewMode !== 'week' || !weekScrollContainerRef.current) return;
    const observer = new ResizeObserver(entries => {
      for (let entry of entries) {
        setDayWidth((entry.contentRect.width - 64) / 7); // 全体の幅から時間軸(64px)を引き、7で割る
      }
    });
    observer.observe(weekScrollContainerRef.current);
  ...
  useEffect(() => {
    if (viewMode === 'week' && weekScrollContainerRef.current) {
      ...
      const currentDayWidth = (weekScrollContainerRef.current.clientWidth - 64) / 7;
      if (targetColIndex !== -1) weekScrollContainerRef.current.scrollTo({ left: targetColIndex * currentDayWidth, behavior: 'auto' });
      ...
      if (targetColIndex !== -1) {
        ...
        dayScrollContainerRef.current.scrollTo({ left: targetColIndex * width, behavior: 'auto' });
      }
    ...
    if (scrollTrigger) {
      if (viewMode === 'week' && weekScrollContainerRef.current) {
        ...
        if (scrollTrigger.direction === 'today') {
          ...
          if (targetColIndex !== -1) weekScrollContainerRef.current.scrollTo({ left: targetColIndex * currentDayWidth, behavior: 'smooth' });
        } else {
          weekScrollContainerRef.current.scrollBy({ left: scrollTrigger.direction === 'next' ? currentDayWidth * 7 : -currentDayWidth * 7, behavior: 'smooth' });
        }
      } else if (viewMode === 'day' && dayScrollContainerRef.current) {
        if (scrollTrigger.direction === 'today') {
          const targetColIndex = days.findIndex(d => d.dayIndex === getDayIndex(new Date()));
          if (targetColIndex !== -1) {
            const width = dayScrollContainerRef.current.clientWidth;
            dayScrollContainerRef.current.scrollTo({ left: targetColIndex * width, behavior: 'smooth' });
          }
        } else {
          const width = dayScrollContainerRef.current.clientWidth;
          dayScrollContainerRef.current.scrollBy({ left: scrollTrigger.direction === 'next' ? width : -width, behavior: 'smooth' });
        }
  ...
  const handleWeekScroll = () => {
    if (viewMode !== 'week' || !weekScrollContainerRef.current) return;
    const currentDayWidth = (weekScrollContainerRef.current.clientWidth - 64) / 7;
    const visibleColIndex = Math.max(0, Math.floor((weekScrollContainerRef.current.scrollLeft + (currentDayWidth / 2)) / currentDayWidth));
    ...
    <main className="flex-1 flex flex-col min-w-0 z-0 relative select-none bg-white">
      ...
      {viewMode === 'week' && (
        <WeekView 
          ...
          resizingEvent={resizingEvent} setResizingEvent={setResizingEvent} weekStartDay={weekStartDay} dayWidth={dayWidth} // ★ 追加
        />
      )}

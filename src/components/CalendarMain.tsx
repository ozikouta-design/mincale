...
interface CalendarMainProps {
  currentMonthYear: string; setCurrentMonthYear: (month: string) => void; currentViewDate: Date; viewMode: "day" | "week" | "month"; setViewMode: (mode: "day" | "week" | "month") => void; scrollTrigger: any; days: any[]; hours: string[]; isLoadingData: boolean; events: any[]; selectedMemberIds: string[]; members: any[];
  handleDragOver: (e: React.DragEvent<HTMLDivElement>) => void; handleDrop: (e: React.DragEvent<HTMLDivElement>, dayIndex: number, startHour: number) => void; handleRangeSelect: (dayIndex: number, startHour: number, duration: number) => void; setIsScheduleModalOpen: (isOpen: boolean) => void; handlePrevWeek: () => void; handleNextWeek: () => void; handleEventClick: (event: any, e: React.MouseEvent) => void; handleEventDragStart: (e: React.DragEvent<HTMLDivElement>, eventId: any, isGoogle: boolean, memberId: string) => void; setIsSidebarOpen: React.Dispatch<React.SetStateAction<boolean>>; setIsRightPanelOpen: React.Dispatch<React.SetStateAction<boolean>>; handleToday: () => void;
}

export default function CalendarMain({
  currentMonthYear, setCurrentMonthYear, currentViewDate, viewMode, setViewMode, scrollTrigger, days, hours, isLoadingData, events, selectedMemberIds, members, handleDragOver, handleDrop, handleRangeSelect, setIsScheduleModalOpen, handlePrevWeek, handleNextWeek, handleEventClick, handleEventDragStart, setIsSidebarOpen, setIsRightPanelOpen, handleToday
}: CalendarMainProps) {
              ...
              <div className="flex-1 flex relative">
                ...
                {day.isToday && (
                  <div className="absolute left-0 right-0 z-20 pointer-events-none flex items-center" style={{ top: `${currentHourExact * 64}px` }}>
                    ...
                    <div className="h-[2px] bg-red-500 w-full shadow-[0_0_4px_rgba(239,68,68,0.5)]"></div>
                  </div>
                )}
                ...
                {selection && selection.dayIndex === day.dayIndex && selection.memberId && (
                  <div className="absolute bg-blue-500/90 rounded-lg shadow-lg pointer-events-none border border-blue-400/50 z-30 px-3 py-2 text-xs text-white overflow-hidden transition-none"
                    style={{ left: `${(activeMembers.findIndex(m => m.id === selection.memberId) / activeMembers.length) * 100}%`, width: `${100 / activeMembers.length}%`, top: `${Math.min(selection.startHour, selection.currentHour) * 64}px`, height: `${Math.max(0.15, Math.abs(selection.currentHour - selection.startHour)) * 64}px` }}
                  >
                ...
                {activeMembers.map((member) => (
                  <div key={member.id} className="flex-1 min-w-[120px] border-r border-gray-50 relative group">
                    {hours.map((_, i) => (
                      <div key={i} className={`h-16 border-b border-gray-50 cursor-crosshair hover:bg-gray-50/30 transition-colors ${dragOverSlot?.dayIndex === day.dayIndex && dragOverSlot?.startHour === i ? 'bg-blue-100/60 ring-2 ring-blue-500 ring-inset z-20 shadow-inner' : ''}`}
                        onMouseDown={(e) => {
                    ...
                    {events.filter(ev => ev.dayIndex === day.dayIndex && ev.memberId === member.id).map((event, idx) => {
                        ...
                        return (
                          <div key={`${event.id}-${idx}`} draggable={true} onMouseDown={(e) => e.stopPropagation()} 
                            ...
                            style={{ top: `${event.startHour * 64 + 1}px`, height: `${event.duration * 64 - 2}px`, left: `calc(${leftPct}% + 1px)`, width: `calc(${widthPct}% - 2px)`, backgroundColor: event.colorHex || member.colorHex }}
                          >
        ...
        <div className="flex-1 flex relative" ref={weekGridRef}>
          ...
          {days.map((day, colIndex) => (
            <div key={day.dayIndex} className={`w-48 shrink-0 border-r border-gray-100 relative ${day.isToday ? 'bg-blue-50/10' : ''}`}>
              {hours.map((_, i) => (
                <div key={i} className={`h-16 border-b border-gray-100 cursor-crosshair hover:bg-gray-50/50 ${dragOverSlot?.dayIndex === day.dayIndex && dragOverSlot?.startHour === i ? 'bg-blue-100/60 ring-2 ring-blue-500 ring-inset z-20 shadow-inner' : ''}`}
                  onMouseDown={(e) => {
              ...
              {events.filter(ev => ev.dayIndex === day.dayIndex && selectedMemberIds.includes(ev.memberId)).map((event, idx) => {
                  ...
                  return (
                    <div key={`${event.id}-${idx}`} draggable={true} onMouseDown={(e) => e.stopPropagation()} 
                      ...
                      style={{ top: `${event.startHour * 64 + 1}px`, height: `${event.duration * 64 - 2}px`, left: `calc(${leftPct}% + 1px)`, width: `calc(${widthPct}% - 2px)`, backgroundColor: event.colorHex || member?.colorHex || "#f97316" }} 
                    >
                      <div className="font-semibold truncate">{event.title}</div>
                      <div className="text-[9px] opacity-90 truncate mt-0.5 flex items-center"><span className="w-1.5 h-1.5 rounded-full bg-white mr-1 opacity-80"></span>{member?.name || "カレンダー"}</div>
                    </div>
                  );
                })}
            </div>
          ...
          {selection && viewMode === 'week' && (
            <div className="absolute bg-blue-500/90 rounded-lg shadow-lg pointer-events-none border border-blue-400/50 z-30 px-3 py-2 text-xs text-white overflow-hidden transition-none"
                 style={{ left: `calc(4rem + ${selection.colIndex * 192}px + 2px)`, width: `calc(192px - 4px)`, top: `${Math.min(selection.startHour, selection.currentHour) * 64}px`, height: `${Math.max(0.15, Math.abs(selection.currentHour - selection.startHour)) * 64}px` }}>
              <div className="font-bold tracking-wide">新規予定</div>
              <div className="text-[10px] opacity-90 mt-0.5">
                {(() => {
                  const totalMinutes = Math.round(Math.min(selection.startHour, selection.currentHour) * 60);
              ...
              {mData.calendarDays.map((d, i) => {
                ...
                return (
                  <div key={i} className={`border-b border-r border-gray-100 p-1 flex flex-col relative hover:bg-gray-50 transition-colors group ${!isCurrentMonth && 'bg-gray-50/40'}`} onClick={() => handleRangeSelect(dayIdx, 0, 1)} onDragOver={(e) => handleDragOver(e)} onDrop={(e) => handleDrop(e, dayIdx, 0)}>
                    <div className={`text-[11px] w-6 h-6 mx-auto rounded-full flex items-center justify-center mb-1 ${isToday ? 'bg-blue-600 text-white font-bold' : isCurrentMonth ? 'text-gray-700 font-medium' : 'text-gray-300'}`}>{d.getDate() === 1 ? `${d.getMonth() + 1}/${d.getDate()}` : d.getDate()}</div>

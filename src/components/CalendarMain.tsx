...
export default function CalendarMain({
  ...
  useEffect(() => {
    if (viewMode === 'week' && scrollTrigger && weekScrollContainerRef.current) {
      if (scrollTrigger.direction === 'today') {
        ...
        if (targetColIndex !== -1) {
          weekScrollContainerRef.current.scrollTo({ left: Math.max(0, targetColIndex * 192 - 192), behavior: 'smooth' });
        }
      } else {
  ...
  const handleWeekScroll = () => {
    ...
    if (days[visibleColIndex]) {
      setCurrentMonthYear(`${days[visibleColIndex].date.getFullYear()}年 ${days[visibleColIndex].date.getMonth() + 1}月`);
    }
  };
  ...
  const renderDayView = () => {
    ...
    return (
      <div 
        className="flex-1 overflow-x-auto overflow-y-auto flex snap-x snap-mandatory bg-white relative"
          ...
          <div key={day.dayIndex} className="min-w-full flex-shrink-0 snap-start flex flex-col h-full bg-white relative">
            ...
            <div className="flex border-b border-gray-200 sticky top-0 bg-white z-40">
              <div className="w-16 shrink-0 border-r border-gray-100 bg-white flex flex-col items-center justify-center py-2">
                <span className={`text-2xl font-light ${day.isToday ? 'font-bold' : 'text-gray-700'}`} style={day.isToday ? { color: accentColor } : {}}>
                  {day.date.getDate()}
                </span>
                <span className={`text-xs ${day.isToday ? 'font-bold' : 'text-gray-500'}`} style={day.isToday ? { color: accentColor } : {}}>
              ...
              <div className="flex-1 flex overflow-hidden">
                {activeMembers.map(member => (
                  <div key={member.id} className="flex-1 min-w-[120px] py-2 border-r border-gray-100 flex flex-col items-center justify-center">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs mb-1 shadow-sm" style={{ backgroundColor: member.colorHex }}>
                      {member.initials}
                    </div>
                    <span className="text-[11px] font-semibold text-gray-700 truncate px-2 w-full text-center">
                      {member.name}
                    </span>
                  </div>
                ))}
            ...
            <div className="flex-1 relative flex" ref={el => { dayGridRefs.current[day.dayIndex] = el; }}>
              <div className="w-16 shrink-0 border-r border-gray-100 bg-white z-20">
                {hours.map((hour, i) => (
                  <div key={i} className="text-right pr-2 py-2 text-[10px] text-gray-400 border-b border-gray-50" style={{ height: hourHeight }}>
                    {hour}
                  </div>
                ))}
              </div>
                ...
                {activeMembers.map((member) => (
                  <div key={member.id} className="flex-1 min-w-[120px] border-r border-gray-50 relative group">
                    {hours.map((_, i) => (
                      <div 
                        key={i} 
                        className={`border-b border-gray-50 cursor-crosshair hover:bg-gray-50/30 transition-colors ${dragOverSlot?.dayIndex === day.dayIndex && dragOverSlot?.startHour === i ? 'ring-2 ring-inset z-20 shadow-inner' : ''}`}
                        style={{
                          height: hourHeight,
                          ...(dragOverSlot?.dayIndex === day.dayIndex && dragOverSlot?.startHour === i ? { backgroundColor: accentColor + '20', borderColor: accentColor } : {})
                        }}
                        onMouseDown={(e) => {
                        ...
                        return (
                          <div 
                            ...
                            style={{ 
                              ...
                              height: `${event.duration * hourHeight - 2}px`, 
    ...
    <div className="flex-1 overflow-x-auto overflow-y-auto flex flex-col bg-white relative" ref={weekScrollContainerRef} onScroll={handleWeekScroll}>
      <div className="flex flex-col min-w-max">
        ...
        <div className="flex border-b border-gray-200 sticky top-0 bg-white z-40">
          ...
          {days.map((day) => (
            <div key={day.dayIndex} className={`w-48 shrink-0 py-3 text-center border-r border-gray-100`} style={day.isToday ? { backgroundColor: accentColor + '10' } : {}}>
              <span className={`text-sm font-medium ${day.isToday ? 'px-3 py-1 rounded-full text-white' : 'text-gray-600'}`} style={day.isToday ? { backgroundColor: accentColor } : {}}>
                {day.label}
              </span>
            </div>
          ))}
        ...
        <div className="flex-1 flex relative" ref={weekGridRef}>
          <div className="w-16 shrink-0 sticky left-0 bg-white z-30 border-r border-gray-100 flex flex-col">
            {hours.map((hour, i) => (
              <div key={i} className="text-right pr-2 py-2 text-[10px] text-gray-400 border-b border-gray-100" style={{ height: hourHeight }}>
                {hour}
              </div>
            ))}
          </div>
          ...
          {days.map((day, colIndex) => (
            <div key={day.dayIndex} className={`w-48 shrink-0 border-r border-gray-100 relative`} style={day.isToday ? { backgroundColor: accentColor + '05' } : {}}>
              {hours.map((_, i) => (
                <div 
                  key={i} 
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        ...
                        left: `calc(${leftPct}% + 1px)`, 
                        width: `calc(${widthPct}% - 2px)`,
                        backgroundColor: event.colorHex || member?.colorHex || accentColor 
                      }} 
                    >
        ...
        {monthsData.map((mData, mIdx) => (
          <div key={mIdx} className="min-w-full flex-shrink-0 snap-start flex flex-col h-full bg-white border-r border-gray-200">
            <div className="grid grid-cols-7 border-b border-gray-200 bg-white shrink-0">
              {['日', '月', '火', '水', '木', '金', '土'].map(w => (
                <div key={w} className="py-2 text-center text-[10px] font-bold text-gray-500 uppercase tracking-widest border-r border-gray-100 last:border-0">
                  {w}
                </div>
              ))}
            </div>
            <div className="flex-1 grid grid-cols-7 grid-rows-6 border-l border-gray-100">
              {mData.calendarDays.map((d, i) => {
                ...
                return (
                  ...
                  >
                    <div 
                      className={`text-[11px] w-6 h-6 mx-auto rounded-full flex items-center justify-center mb-1 ${isToday ? 'text-white font-bold' : isCurrentMonth ? 'text-gray-700 font-medium' : 'text-gray-300'}`}
                    ...
                    <div className="flex-1 flex flex-col gap-0.5 overflow-hidden">
                      {visibleEvents.map((e, evIdx) => {
                        ...
                        const bgColor = e.colorHex || member?.colorHex || accentColor;
                        return (
                          <div 
                            key={evIdx} 
                            draggable={true} 
          ...
          <div className="hidden md:flex items-center bg-gray-100 rounded-lg p-1 border border-gray-200 shrink-0">
            {["day", "week", "month"].map((mode) => (
              <button 
                ...
                className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${viewMode === mode ? 'bg-white shadow-sm' : 'text-gray-500 hover:text-gray-800'}`}
                style={viewMode === mode ? { color: accentColor } : {}}
              >

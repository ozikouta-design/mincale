...
export default function CalendarHeader({
  ...
  return (
    <header className="flex flex-col border-b border-gray-200 bg-white shrink-0 z-30 shadow-sm relative">
      {/* メインヘッダー行 */}
      <div className="h-[60px] md:h-16 flex items-center justify-between px-2 md:px-6">
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick={() => setIsSidebarOpen(true)} className="md:hidden p-1.5 text-gray-500 hover:bg-gray-100 rounded-lg">
            <Menu className="w-5 h-5" />
          </button>
          <h2 className="text-base md:text-2xl font-black text-gray-800 tracking-tight min-w-[80px] md:min-w-[140px] truncate">
            {displayMonthYear}
          </h2>
      ...
      {/* モバイル専用 第2行: 今日 + 表示切替 */}
      <div className="flex lg:hidden items-center justify-between px-3 pb-2 gap-2">
        <button
          onClick={handleToday}
          className="px-3 py-1 text-sm font-bold text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 active:bg-gray-100 transition-all shadow-sm"
        >
          今日
        </button>
        <div className="flex items-center bg-gray-100 p-0.5 rounded-xl border border-gray-200/50">
          {(["day", "week", "month"] as const).map((mode) => (
            <button
              key={mode}
              onClick={() => setViewMode(mode)}
              className={`px-4 py-1.5 text-sm font-bold rounded-xl transition-all duration-200 ${

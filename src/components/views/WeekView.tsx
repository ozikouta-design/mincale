      ...
      <div className="flex flex-col min-w-max">
        <div className="flex border-b border-gray-200 sticky top-0 bg-white z-40">
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day.dayIndex} className={`shrink-0 py-2 md:py-3 text-center border-r border-gray-100 ${day.date.getDay() === weekStartDay ? 'snap-start snap-always' : ''}`} style={{ width: dayWidth, ...(day.isToday ? { backgroundColor: accentColor + '10' } : {}) }}>
                <div className={`w-8 h-8 md:w-auto md:h-auto mx-auto rounded-full flex flex-col items-center justify-center ${day.isToday ? 'shadow-sm' : ''}`} style={day.isToday ? { backgroundColor: accentColor } : {}}>
        ...
        <div className="flex-1 flex relative" ref={weekGridRef}>
          ...
          {selection && (
            <div className="absolute rounded-lg shadow-lg pointer-events-none z-30 px-3 py-2 text-xs text-white overflow-hidden transition-none" style={{ backgroundColor: accentColor + 'E6', left: `calc(4rem + ${(selection.colIndex / 7) * 100}% + 2px)`, width: `calc(${100 / 7}% - 4px)`, top: `${Math.min(selection.startHour, selection.currentHour) * hourHeight}px`, height: `${Math.max(0.15, Math.abs(selection.currentHour - selection.startHour)) * hourHeight}px` }}>

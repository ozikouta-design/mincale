...
export default function WeekView({
  ...
  weekScrollContainerRef, handleWeekScroll, weekGridRef
}: WeekViewProps) {
  return (
    <div className="flex-1 overflow-x-auto overflow-y-auto flex flex-col bg-white relative" ref={weekScrollContainerRef} onScroll={handleWeekScroll}>
      <div className="flex flex-col min-w-max">
        <div className="flex border-b border-gray-200 sticky top-0 bg-white z-40">
          ...
          {days.map((day) => (
            <div key={day.dayIndex} className={`w-48 shrink-0 py-3 text-center border-r border-gray-100`} style={day.isToday ? { backgroundColor: accentColor + '10' } : {}}>
              <span className={`text-sm font-medium ${day.isToday ? 'px-3 py-1 rounded-full text-white' : 'text-gray-600'}`} style={day.isToday ? { backgroundColor: accentColor } : {}}>{day.label}</span>
            </div>
          ))}
        </div>
        <div className="flex-1 flex relative" ref={weekGridRef}>
          ...
          {selection && (
            <div className="absolute rounded-lg shadow-lg pointer-events-none z-30 px-3 py-2 text-xs text-white overflow-hidden transition-none" style={{ backgroundColor: accentColor + 'E6', left: `calc(4rem + ${selection.colIndex * 192}px + 2px)`, width: `calc(192px - 4px)`, top: `${Math.min(selection.startHour, selection.currentHour) * hourHeight}px`, height: `${Math.max(0.15, Math.abs(selection.currentHour - selection.startHour)) * hourHeight}px` }}>

...
export function useCalendarLogic() {
  ...
  const handleSaveBookingSettings = async () => {
    ...
    const { error } = await supabase.from('profiles').update({ booking_duration: bookingDuration, booking_start_hour: bookingStartHour, booking_end_hour: bookingEndHour, booking_days: bookingDays, booking_lead_time: bookingLeadTime, week_start_day: weekStartDay }).eq('email', session.user.email);
  ...
  const syncGoogleData = async () => {
    ...
    setIsSyncing(true);
    try {
      const token = (session as any).accessToken; const calListRes = await fetch("https://www.googleapis.com/calendar/v3/users/me/calendarList", { headers: { Authorization: `Bearer ${token}` } });
      if (!calListRes.ok) throw new Error("カレンダーリスト取得失敗"); const calListData = await calListRes.json();
      ...
      if (calListData.items && calListData.items.length > 0) {
        ...
        setMembers(fetchedMembers); if (selectedMemberIds.length === 0) { setSelectedMemberIds(fetchedMembers.map(m => m.id)); setNewEventMemberId(defaultPrimaryId || fetchedMembers[0].id); }
      }
